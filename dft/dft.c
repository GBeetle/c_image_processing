...
int main(int argc, char const *argv[])
{
    ...
    bmheader.height = h;
    bmheader.width  = w;
    re_array = allocate_double_array(h, w);
    im_array = allocate_double_array(h, w);

    create_allocate_bmp_file(argv[2], &bmp_file_header, &bmheader);

    // dft(in_array, out_array, h, w);
    dft(in_array, re_array, im_array, h, w);
    idft(re_array, im_array, out_array, h, w);
    // fre_spectrum(in_array, out_array, h, w);
    write_bmp_image(argv[2], out_array);

    free_image_array(in_array, h);
    free_image_array(out_array, h);
    free_double_array(re_array, h);
    free_double_array(im_array, h);
